...
* **Spotlight Universal B2B:** Se implementó un buscador flotante centralizado accesible mediante la tecla `Enter` y/o `Doble Clic` en selectores nativos o inputs autocompletables. Desacopla la vista de selectores engorrosos por una interfaz enfocada (`rxn-spotlight.js`) con interpolación de Capture Events prioritarios, soportando tanto peticiones remotas (`data-picker-url`) como extracción local de etiquetas opciones (`<select>`).
* **Documentación Módulo Sync:** Se auditaron las reglas técnicas de `RxnSync` documentando el patrón *Shadow Copy* y los límites de *Match Suave* (heurística de paginación) dentro de su propio `MODULE_CONTEXT.md` para resguardar la arquitectura del CRM en desarrollo.
* **Contexto CrmClientes Validado:** Se redactó el `MODULE_CONTEXT.md` del módulo `CrmClientes`, evidenciando el patrón de inyección de schema on-the-fly y formalizando el endpoint JSON de sugerencias que sostiene a Presupuestos y Pedidos de Servicio.
