    ...
    <div class="user-grid" id="userGrid">
        ...
        <?php else: ?>
            <?php foreach ($usuarios as $u): ?>
                <div class="user-card rxn-searchable-item" style="cursor: grab;" data-id="<?= htmlspecialchars((string)$u['id']) ?>" data-search-content="<?= htmlspecialchars(strtolower($u['nombre'] . ' ' . $u['email'] . ' ' . $u['rol'])) ?>">
                    <div class="user-info-wrapper">
                        <div style="position: relative;">
                            <div class="user-avatar <?= htmlspecialchars($u['avatar_color']) ?>">
                                <?= htmlspecialchars($u['iniciales']) ?>
                            </div>
                            <div class="user-status-dot <?= $u['is_current_user'] ? 'active' : ($u['activo'] ? 'inactive' : 'inactive opacity-50') ?>" title="<?= $u['is_current_user'] ? 'Tú (En línea)' : ($u['activo'] ? 'Cuenta de sistema activa' : 'Cuenta suspendida') ?>"></div>
                        </div>
