...
- **Creación de PDS/Presupuestos/Notas desde una tratativa**: se usa el patrón de query param `?tratativa_id=X[&cliente_id=Y]`, idéntico al flujo "Generar PDS desde Llamadas" (`CrmLlamadas → CrmPedidosServicio`). Los controllers destino leen el query string y persisten la vinculación en el registro. Al guardar, el redirect **se queda en el propio documento** (`/editar`); el retorno al detalle de la tratativa se hace vía botón Volver (ver regla siguiente).
- **Volver contextual + Escape = Volver** (patrón transversal PDS/Presupuestos/Notas/Tratativas): el form/detalle de Tratativas y los documentos hijos (PDS, Presupuesto, Nota) declaran su botón Volver con el atributo `data-rxn-back` y un href calculado por la vista. El script global `public/js/rxn-escape-back.js` escucha Escape y, si no hay input/modal en foco, navega a ese href. Reemplaza el `history.back()` nativo por una navegación explícita al destino que la vista considera "volver".
