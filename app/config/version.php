...
return [
    ...
    'current_build' => '20260506.2',
    'history' => [
        [
            'version' => '1.47.0',
            'build' => '20260506.2',
            'released_at' => '2026-05-06',
            'title' => 'Iteración 51 — Matriz de permisos modulares (empresa + usuario) + UX click-to-center en GeoTracking',
            'summary' => 'Release grande de control de acceso. Hasta ahora los toggles de módulos eran solo a nivel empresa (4 flags en empresas: CRM, Notas, Llamadas, Monitoreo, RXN Live) y los permisos de usuario eran binarios globales (es_admin / es_rxn_admin). En esta iteración construimos una matriz de 2 niveles que cubre los 11 módulos contratables del CRM. (Nivel 1 — Empresa) Agregamos 7 toggles nuevos en la tabla empresas: crm_modulo_pedidos_servicio, crm_modulo_agenda, crm_modulo_mail_masivos, crm_modulo_horas_turnero, crm_modulo_geo_tracking, crm_modulo_presupuestos_pwa, crm_modulo_horas_pwa. Migración idempotente con DEFAULT 0 (empresa nueva no tiene nada contratado hasta que el super admin lo habilite explícitamente). EmpresaAccessService extendido con 7 métodos hasCrmXxxAccess() + 7 requireCrmXxxAccess() siguiendo el patrón existente. Vista /empresas/:id/editar y /empresas/crear suman los 7 switches en el bloque CRM, con dependencia en cascada del switch padre crm. EmpresaRepository::save y update + EmpresaService::create/update/copy persisten los nuevos campos. (Nivel 2 — Usuario) Nueva matriz por usuario: 11 columnas TINYINT en usuarios (usuario_modulo_*) con DEFAULT 1 — todos los usuarios existentes y nuevos arrancan con todos los módulos habilitados, el admin de empresa los va apagando si quiere restringir. UI en Administrar Cuentas → Editar Usuario: bloque "Módulos habilitados" que solo muestra los módulos contratados a nivel empresa (filtro automático), con disabled si el editor no es admin. Defensa server-side en UsuarioService::applyModuleFlags(): aunque alguien manipule el HTML y mande flags por POST, el service descarta los cambios si el editor no tiene canManageAdminPrivileges(). Tabla nueva usuarios_modulos_audit + trigger MySQL AFTER UPDATE que captura cambios en cualquiera de los 11 flags y registra before/after en JSON con atribución vía @audit_user_id. Service nuevo App\\Modules\\Auth\\UserModuleAccessService con userHas() y requireUserAccess(); bypass automático para es_admin y es_rxn_admin. (Hardening) Endurecidos 11 endpoints de RxnPwaController + 5 controllers operativos (CrmAgenda, CrmHoras turnero, RxnGeoTracking dashboard+config, CrmMailMasivos en sus 4 controllers, CrmPedidosServicio) con doble compuerta empresa+usuario. Launcher de RxnPwa ahora se construye dinámicamente: cards de Presupuestos PWA y Horas PWA solo aparecen si la empresa las contrata Y el usuario las tiene habilitadas. (Bugfix anti-auto-baja) Bug pre-existente que el feature destapó: el toggle "Usuario activo" era editable libremente, dejando que un usuario se auto-desactive o que un operador desactive a otros. Fix doble: UI muestra el toggle disabled con mensaje claro (self-edit O editor no admin), y UsuarioService::update preserva el valor del DB ignorando el POST si las reglas no se cumplen. (UX GeoTracking) Bonus pedido por Charly al cierre: cada fila de la tabla de eventos en el dashboard de GeoTracking ahora es clickable. Click → map.panTo() a la lat/lng exacta + zoom 16 + scroll suave al mapa + abre el popup del marker correspondiente con los datos completos. Filas sin lat/lng (denied/error) no son clickables. Si el evento está fuera del límite de 500 puntos del mapa, se construye un popup mínimo con datos de la fila. Validación: php -l limpio en los 19 archivos tocados. JS syntax verificado con node. Las dos migraciones nuevas (empresas + usuarios+audit+trigger) corridas idempotentes en local sin warnings. Próxima iteración: extender la matriz a más módulos + auditoría formal de seguridad (punto B pendiente: RxnLive, Notifications, Drafts, RxnGeoTracking, CrmHoras PWA + mapping ASVS L2).',
            'items' => [
                'database/migrations/2026_05_06_03_add_modulos_extendidos_to_empresas.php: 7 columnas TINYINT NOT NULL DEFAULT 0 en empresas (pedidos_servicio, agenda, mail_masivos, horas_turnero, geo_tracking, presupuestos_pwa, horas_pwa). Idempotente (information_schema check antes de cada ALTER).',
                'database/migrations/2026_05_06_04_add_modulos_usuarios_audit_trigger.php: 11 columnas TINYINT NOT NULL DEFAULT 1 en usuarios + tabla usuarios_modulos_audit (id, usuario_id, empresa_id, flags_before JSON, flags_after JSON, changed_by, changed_at) + trigger tr_usuarios_modulos_audit_after_update que solo loggea si cambia al menos uno de los 11 flags. Idempotente.',
                'app/modules/Empresas/Empresa.php: 7 propiedades nuevas. EmpresaRepository::save/update/_mapRowToEntity persisten los 7 campos. EmpresaService::create/update/copy sumamos los 7 (con cascada en modulo_crm).',
                'app/modules/Empresas/EmpresaAccessService.php: 7 hasCrmXxxAccess() + 7 requireCrmXxxAccess() siguiendo patrón existente.',
                'app/modules/Empresas/views/editar.php y crear.php: 7 switches nuevos en el bloque CRM, con dependencia data-empresa-subdependiente="crm" (cascada del padre).',
                'app/modules/Auth/Usuario.php: 11 propiedades nuevas usuario_modulo_* con DEFAULT 1.',
                'app/modules/Auth/UsuarioRepository.php: 11 ALTERs idempotentes en constructor (defensiva, mismo patrón que tango_perfil_*). INSERT y UPDATE persisten los 11 flags. Antes del UPDATE setea @audit_user_id desde $_SESSION para que el trigger tenga atribución.',
                'app/modules/Auth/UserModuleAccessService.php: nuevo. current()/userHas()/requireUserAccess()/flags(). Bypass automático para es_admin y es_rxn_admin (admin de empresa ve todos los módulos contratados sin tildarse).',
                'app/modules/Usuarios/UsuarioService.php: applyModuleFlags() descarta cambios si !canManageAdminPrivileges. Llamado desde create() y update(). Bug fix anti-auto-baja: toggle activo solo se procesa si !isSelfEdit && canManageAdminPrivileges, sino preserva el valor del DB.',
                'app/modules/Usuarios/UsuarioController.php: pasa empresaTarget a las vistas (crear/editar/store/update) para filtrar qué módulos mostrar según contratado de empresa.',
                'app/modules/Usuarios/views/editar.php y crear.php: bloque "Módulos habilitados" con los 11 toggles, filtrados por contratado de empresa, disabled si no es admin. Toggle "Usuario activo" disabled con mensaje claro si self-edit o no admin.',
                'app/modules/RxnPwa/RxnPwaController.php: 2 helpers privados requirePresupuestosPwa() / requireHorasPwa(). 11 endpoints PWA endurecidos. Launcher construye $pwaApps dinámicamente desde EmpresaAccessService + UserModuleAccessService.',
                'app/modules/RxnPwa/views/launcher.php: array hardcoded reemplazado por iteración sobre $pwaApps que viene del controller. Mensaje friendly cuando no hay módulos disponibles.',
                'app/modules/CrmAgenda/AgendaController.php: helper privado requireAgendaAccess() (login + módulo empresa + módulo usuario). Aplicado en 12 métodos públicos.',
                'app/modules/CrmHoras/HoraController.php: helper requireHorasTurneroAccess() reemplaza el par requireLogin+requireCrmAccess en 12 métodos.',
                'app/modules/RxnGeoTracking/RxnGeoTrackingController.php y RxnGeoTrackingConfigController.php: helper requireGeoTrackingAccess() (requireBackofficeAdmin + módulo empresa + módulo usuario). Endpoints Consent y Report quedan sin chequeo de módulo (cualquier usuario logueado puede reportar su propia posición).',
                'app/modules/CrmMailMasivos/{MailMasivosDashboardController,TemplateController,ReportController,JobController}.php: cada AuthService::requireLogin() seguido de requireCrmMailMasivosAccess() + requireUserAccess. TrackingController NO tocado (recibe webhooks externos, no requiere login).',
                'app/modules/CrmPedidosServicio/PedidoServicioController.php: helper requirePedidosServicioAccess() en los 18 métodos públicos.',
                'app/modules/RxnGeoTracking/views/dashboard.php: filas con lat/lng tienen data-event-id, data-lat, data-lng + clase rxn-geo-row-clickable. CSS sumado para cursor pointer + hover azul + barrita lateral + estado activo.',
                'public/js/rxn-geo-tracking-dashboard.js: markerById Map indexa markers por id; pointById cachea los datos del point para reconstruir popup. focusEvent() centra mapa + zoom 16 + scroll + popup. wireTableRowClicks() vincula clicks de tabla. Fallback popup con datos del row si el evento está fuera del límite de 500.',
                'database/migrations/2026_05_06_05_seed_customer_notes_release_1_47_0.php: seed idempotente con notas visibles al cliente final.',
                'app/config/version.php: bump a 1.47.0 / build 20260506.2.',
            ],
        ],
        [
            'version' => '1.46.4',
            'build' => '20260506.1',
            'released_at' => '2026-05-06',
            'title' => 'Iteración 50 — Auditoría DELETE Presupuestos + endurecimiento CSRF (RxnSync, TangoSync, Attachments)',
            'summary' => 'Sesión doble: arrancamos cerrando el pendiente de la iteración 48 (replicar la auditoría de eliminación permanente que hicimos para PDS en 1.46.3, ahora también para Presupuestos) y seguimos con un barrido de seguridad sobre los 4 módulos críticos detectados pendientes (WebPush, RxnSync, TangoSync, AttachmentsController). El barrido detectó hallazgos serios y los fixeamos en la misma release. (A) Auditoría DELETE Presupuestos: misma red de seguridad triple-capa que PDS — tabla crm_presupuestos_audit_deletes con 34 campos en before_json, trigger SQL BEFORE DELETE que captura cualquier delete (PHP, phpMyAdmin, SQL manual), atribución vía @audit_user_id/@audit_user_name seteadas por el repository antes del DELETE, vista RXN_LIVE_VW_PRESUPUESTOS_DELETES con flag estaba_en_tango calculado sobre nro_comprobante_tango, dataset presupuestos_eliminados en RxnLive. Smoke test 4 casos verde end-to-end. (B) TangoSyncController: los 9 endpoints de sync (clientes/articulos/precios/stock/todo × tiendas+crm) eran GET — eran explotables vía CSRF triggable por <img src="…"> en mail HTML o sitio externo, sin que la víctima tuviera que hacer click ni interactuar. Pasaron a POST + verifyCsrfOrAbort en el Controller base + sanitización de getMessage en los 5 catches (log server-side detallado, mensaje genérico al usuario). Las dos <a href> de Sync Precios y Sync Stock en RxnSync/views/index.php convertidas a <form method=POST> con CsrfHelper::input(). (C) RxnSyncController: 11 endpoints AJAX POST sin CSRF + getMessage filtrado + error_class/error_file en JSON (reconnaissance gratis para atacante). Helper privado verifyCsrfHeaderOrAbortJson valida header X-CSRF-Token y devuelve 419 JSON con kind:"csrf"; helper logAndGenericError centraliza log+respuesta limpia. JS del módulo agrega header X-CSRF-Token a los 3 fetch POST leyendo del meta name="csrf-token". (D) AttachmentsController: upload() y delete() filtraban $e->getMessage() directo al cliente — ahora distinguen InvalidArgumentException/RuntimeException (mensaje user-friendly seguro del service) de Throwable genérico (log server-side + "Error interno..."). (E) Documentación reforzada: docs/seguridad/convenciones.md suma sección 3.4 con el patrón verifyCsrfHeaderOrAbortJson para AJAX, sección 3.5 con la regla "POST para acciones, nunca GET" + ejemplo de conversión de <a href> a <form>, sección 6.3 ampliada con el patrón de sanitización de catches. La checklist final del módulo nuevo suma 3 ítems para que todo módulo futuro nazca con estos estándares — la idea es no repetir esta pasada. MODULE_CONTEXT.md actualizados: CrmPresupuestos suma sección "Auditoría de eliminación permanente (1.46.4)" análoga a PDS; RxnSync y Tango pasan de "Sin CSRF — deuda activa" a la nueva sección con el patrón implementado. Validación end-to-end vía cURL + smoke test PHP: rebote correcto sin token (419/500 con body coherente), aceptación con token, JSON limpio sin filtración. Pospuestos para próxima iteración: auditoría formal módulo por módulo de los 5 restantes (RxnLive, Notifications, Drafts, RxnGeoTracking, CrmHoras PWA) + mapping ASVS L2 consolidado.',
            'items' => [
                'database/migrations/2026_05_06_01_create_crm_presupuestos_audit_deletes.php: tabla crm_presupuestos_audit_deletes (34 campos en before_json, columnas explícitas para nro_comprobante_tango, total, estado, version_numero, tratativa_id, etc) + trigger BEFORE DELETE + vista RXN_LIVE_VW_PRESUPUESTOS_DELETES con flag estaba_en_tango calculado. Migración idempotente.',
                'app/modules/CrmPresupuestos/PresupuestoRepository.php: forceDeleteByIds setea @audit_user_id y @audit_user_name (MySQL session vars) antes del DELETE para que el trigger las lea como atribución.',
                'app/modules/RxnLive/RxnLiveService.php: nuevo dataset "presupuestos_eliminados" con pivot_metadata completo (17 campos) y chart agrupado por eliminado_por.',
                'app/config/routes.php: 9 endpoints de TangoSyncController pasaron de $router->get a $router->post (sync/clientes, sync/articulos, sync/precios, sync/stock, sync/todo × tiendas+crm).',
                'app/modules/Tango/Controllers/TangoSyncController.php: cada método llama $this->verifyCsrfOrAbort() al inicio. Catches sanitizados — error_log server-side con clase + getMessage + file:line; al cliente Flash genérico ("Error al sincronizar X. Revisá los logs del servidor."). redirectPath y syncClientes aceptan return también desde POST.',
                'app/modules/RxnSync/RxnSyncController.php: nuevo helper privado verifyCsrfHeaderOrAbortJson() (419 JSON con kind:"csrf" si falla) llamado en los 11 métodos POST. Nuevo helper logAndGenericError() centraliza catch+log+JSON limpio. Eliminado error_class/error_file del JSON de error.',
                'app/modules/RxnSync/views/index.php: helper JS csrfToken() lee del meta name="csrf-token". Header X-CSRF-Token agregado a los 3 fetch POST (runRxnSyncAjax, sync-catalogos, doBulkRequest). Las dos <a href> de Sync Precios/Sync Stock convertidas a <form method=POST> con CsrfHelper::input() y parse_url para preservar el ?return= como hidden field.',
                'app/shared/Controllers/AttachmentsController.php: upload() y delete() distinguen InvalidArgumentException/RuntimeException (mensaje user-friendly del service) de Throwable genérico (log server-side + "Error interno...").',
                'docs/seguridad/convenciones.md: nueva sección 3.4 (patrón verifyCsrfHeaderOrAbortJson para AJAX), nueva sección 3.5 (regla POST vs GET para acciones + ejemplo de conversión <a> → <form>), sección 6.3 ampliada con patrón de sanitización de catches. Checklist final suma 3 ítems (CSRF AJAX, POST vs GET, sanitización de catches). Cierre con bullet sobre audit log de hard-deletes apuntando a los MODULE_CONTEXT de PDS y Presupuestos.',
                'app/modules/CrmPresupuestos/MODULE_CONTEXT.md: nueva sección "Auditoría de eliminación permanente (1.46.4)" análoga a la de PDS, con explicación de la red triple-capa, instrucciones de mantenimiento ante cambio de schema, y referencia al patrón canónico.',
                'app/modules/RxnSync/MODULE_CONTEXT.md: sección CSRF actualizada de "Sin validación — deuda activa" a "CSRF (release 1.46.4) — Resuelto" con el patrón verifyCsrfHeaderOrAbortJson + JS csrfToken() documentado. Nueva sección "Sanitización de errores en JSON (release 1.46.4)" con el helper logAndGenericError.',
                'app/modules/Tango/MODULE_CONTEXT.md: tabla de rutas refleja POST en los 9 endpoints. Sección CSRF actualizada con verifyCsrfOrAbort. Sección de sanitización de errores agregada. "Sin CSRF" eliminado de Riesgos Conocidos.',
                'database/migrations/2026_05_06_02_seed_customer_notes_release_1_46_4.php: seed idempotente con nota visible al cliente final (lenguaje de capacidad, sin detalle técnico).',
                'app/config/version.php: bump a 1.46.4 / build 20260506.1.',
            ],
        ],
        [
            'version' => '1.46.3',
        ...
        [
            ...
            'build' => '20260502.2',
            'released_at' => '2026-05-02',
            'title' => 'Iteración 45 — PWA Horas (turnero) end-to-end + adjuntos en desktop + UX 419 + pickers',
            'summary' => 'Release grande con 4 frentes. (1) PWA mobile de Horas: nuevo turnero offline-first con cronómetro vivo, total trabajado del día en vivo, botón único contextual Iniciar/Cerrar, concepto como textarea, vincular a tratativa (catálogo offline), descuento + motivo, adjuntos con cámara para certificados médicos, sync queue 2-step con backoff a /api/rxnpwa/horas/sync, idempotente por tmp_uuid_pwa. Sin Tango (no aplica al turnero). 2 vistas (horas_shell + horas_form), 4 JS (drafts-store, sync-queue, shell, form), 1 service server-side (RxnPwaHorasSyncService), idempotencia en HoraRepository. (2) Adjuntos en desktop: nueva vista detalle.php (/mi-empresa/crm/horas/{id}) accesible al dueño + admin con upload + delete + lista, link 📎 en cada turno del día desde turnero.php. crm_hora se suma a la whitelist de attachments polimórficos. Concepto pasa a textarea en turnero.php, diferido.php y editar.php (web) con la misma UX que la PWA. (3) UX 419 sesión expirada: AuthController::processLogin con CSRF expirado redirige a /login?expired=1 (en vez de 419 dead-end) con alert amable + token fresco. Página 419 global ahora renderea vista decente con botones "Iniciar sesión nuevamente" (preserva next del Referer validado con isSafeNext) y "Volver atrás". (4) Pickers cliente/artículo: bug estructural en setupPicker que borraba el hidden id al primer tipeo accidental, fixeado con snapshot del par válido (validText/validHidden) que se restaura si el texto vuelve al original. Aplicado en Pedidos y Presupuestos. Bonus: rxn-draft-autosave.js skipea dispatch input/change en pickers al rehidratar (evita que el handler los borre). DB IndexedDB bumpeada a v4 con stores horas_drafts + horas_attachments + tratativas_activas en el catálogo offline. SW bumpeado a v16 con fallback contextual mejorado. Fix de contraste dark mode en geo-gate dev banner y launcher (text-muted invisible sobre fondo dark). Botón "Instalar PWA" híbrido en el launcher: prompt nativo si Chrome lo entrega, modal con instrucciones manuales detectando UA si no (caso HTTP en LAN sin SSL). Botón "Menú PWA" en headers de Horas y Presupuestos para navegar entre apps.',
            'items' => [
                'database/migrations/2026_05_01_01_alter_crm_horas_add_pwa_descuento_motivo.php: nueva migración. Suma a crm_horas: tmp_uuid_pwa VARCHAR(50) NULL UNIQUE (idempotencia sync mobile), descuento_segundos INT NOT NULL DEFAULT 0, motivo_descuento TEXT NULL.',
                'app/modules/CrmHoras/HoraRepository.php: INSERT/UPDATE incluyen los 3 campos nuevos. update() unsetea tmp_uuid_pwa (no se modifica al editar). findByTmpUuidPwa() para idempotencia.',
                'app/modules/CrmHoras/HoraService.php: iniciar/cargarDiferido/editar aceptan descuento_segundos + motivo_descuento. Validación cruzada: si descuento > 0, motivo obligatorio; descuento ≤ duración total. cargarDiferido también acepta tmp_uuid_pwa para sync mobile.',
                'app/modules/CrmHoras/HoraController.php: parsea HH:MM:SS en iniciar/diferidoStore/editarStore. 3 acciones nuevas: detalle (vista nueva accesible al dueño + admin), uploadAdjunto, deleteAdjunto. Helper parseDurationOrZero.',
                'app/modules/CrmHoras/views/turnero.php: concepto pasa a textarea (rows=3, maxlength=2000). Bloque <details> colapsable "Aplicar descuento" con HH:MM:SS + motivo. Cada item de la lista del día tiene link 📎 al detalle. Muestra descuento+motivo si los hay.',
                'app/modules/CrmHoras/views/diferido.php: idem turnero (textarea + descuento collapsible).',
                'app/modules/CrmHoras/views/editar.php: textarea concepto + descuento + motivo + sección Adjuntos completa (lista, upload, delete) usando AttachmentService.',
                'app/modules/CrmHoras/views/detalle.php: NUEVA vista. Resumen del turno (fechas, duración bruta-neta, concepto, descuento, motivo) + sección Adjuntos. Accesible al dueño + admin del tenant. Link "Editar (admin)" si corresponde.',
                'app/config/routes.php: 3 rutas nuevas crm_horas (detalle GET, adjuntos POST, adjuntos/borrar POST) + 5 rutas pwa (horas shell/nuevo/editar, /api/rxnpwa/horas/sync, /api/rxnpwa/horas/{id}/attachments).',
                'app/config/attachments.php: crm_hora sumado al whitelist de allowed_owner_types (junto a crm_nota y crm_presupuesto).',
                'app/modules/RxnPwa/RxnPwaController.php: 5 métodos nuevos para Horas (horasShell, horasNuevo, horasEditar, syncHora, uploadHoraAttachment). Reusa la infra de geo-gate, csrf y rate limiter del controller existente.',
                'app/modules/RxnPwa/RxnPwaHorasSyncService.php: NUEVO. Mapea draft IndexedDB → HoraService::cargarDiferido. Idempotencia por tmp_uuid_pwa con doble check (race condition). Captura geo del cliente PWA si vino.',
                'app/modules/RxnPwa/RxnPwaCatalogService.php: suma fetchTratativasActivas (top 100, estados nueva/en_curso/pausada) al payload del catálogo offline. CATALOG_SCHEMA_VERSION → v3.',
                'app/modules/RxnPwa/views/horas_shell.php: NUEVA. Header con botón Menú PWA + fullscreen + volver. Total trabajado hoy en vivo (suma turnos cerrados + cronómetro abierto en tiempo real). Botón único contextual Iniciar/Cerrar. Concepto + tratativa + descuento al iniciar. Lista de turnos del día con badges de estado (EN CURSO / Borrador / Pendiente / Sincronizado). Cola de envío con purga manual de sincronizados.',
                'app/modules/RxnPwa/views/horas_form.php: NUEVA. Form de turno diferido o edición de borrador. Datetime inicio + fin + concepto textarea + tratativa select + descuento + motivo + adjuntos (cámara con capture=environment + file picker, compresión client-side de imágenes). Auto-save debounced 1.5s. Botón sincronizar al final.',
                'public/js/pwa/rxnpwa-catalog-store.js: DB_VERSION bumpeada a 4. Stores nuevas: horas_drafts (keyPath tmp_uuid + indices) + horas_attachments (autoIncrement + index by_tmp_uuid). tratativas_activas sumada al array STORES. loadAll/saveCatalog/clearCatalogOnly/clear ahora son defensivos: filtran stores que NO existen en la DB para no crashear con NotFoundError si el upgrade quedó pendiente.',
                'public/js/pwa/rxnpwa-horas-drafts-store.js: NUEVO. CRUD de drafts con concepto/tratativa/descuento/motivo + adjuntos pendientes con sync_status. findOpenDraft (turno con fecha_inicio sin fecha_finalizado). GC de drafts synced > 7 días.',
                'public/js/pwa/rxnpwa-horas-sync-queue.js: NUEVO. Cola 2-step: POST /api/rxnpwa/horas/sync para el header → POST /api/rxnpwa/horas/{id}/attachments para cada adjunto pendiente. Backoff exponencial 1s/2s/4s/8s/16s, max 5 reintentos. Captura geo automática si está disponible. Auto-trigger en online + DOMContentLoaded + Background Sync.',
                'public/js/pwa/rxnpwa-horas-shell.js: NUEVO. Espejo del desktop turnero con cronómetro vivo (setInterval 1s recalcula total trabajado del día). Detecta turno abierto del día y pinta botón rojo "Cerrar"; sino pinta verde "Iniciar" con form (concepto/tratativa/descuento). Lista del día con acciones (Editar, Sincronizar, Eliminar) y purga manual de sincronizados.',
                'public/js/pwa/rxnpwa-horas-form.js: NUEVO. Bootstrap del form, populate de tratativas desde catálogo offline, autosave debounced, cámara + adjuntos con compresión, validación pre-sync (fechas, descuento+motivo coherente), refresh de estado en cada evento de la cola.',
                'app/modules/Auth/AuthController.php: processLogin con CSRF expirado redirige 302 a /login?expired=1 con next preservado, en vez de abortar con 419. Email NO se preserva en URL (privacidad).',
                'app/modules/Auth/views/login.php: alert warning "Tu pestaña estuvo inactiva mucho tiempo y el formulario expiró" cuando ?expired=1.',
                'app/core/Controller.php: verifyCsrfOrAbort renderiza vista app/core/views/error_419.php en lugar de echo plano. Deriva next desde HTTP_REFERER + parse_url + isSafeNext (whitelist anti open-redirect). Solo acepta hosts iguales al actual.',
                'app/core/views/error_419.php: NUEVA. Branding Reaxion light, mensaje amable, 2 botones: "Iniciar sesión nuevamente" → /login?expired=1&next=<url> y "Volver atrás" → history.back().',
                'public/js/crm-pedidos-servicio-form.js: setupPicker captura validText/validHidden al inicializar y refresca en applyItem(). Handler input restaura el hidden si el texto vuelve al snapshot, solo borra cuando cambia real. Fixea bug de "tipeo accidental + delete deja id vacío → falla validación".',
                'public/js/crm-presupuestos-form.js: mismo patrón, con extraHidden adicional para clasificación.',
                'public/js/rxn-draft-autosave.js: applyPayload skipea dispatchEvent input/change en inputs [data-picker-input]. Coherencia visible↔hidden viene del draft, no hace falta que el handler del picker reaccione (y lo borraría).',
                'app/modules/RxnPwa/views/launcher.php: botón "Instalar app" siempre visible. Si beforeinstallprompt llegó → diálogo nativo. Si no (HTTP local, iOS Safari) → modal Bootstrap con instrucciones manuales detectando UA (Android Chrome / iOS Safari / genérico) + mensaje contextual del por qué no apareció automático. Listener appinstalled para auto-ocultar el card. Fix CSS de contraste: descripciones de cards + hint general + footer ahora legibles sobre fondo dark.',
                'public/manifest.webmanifest: start_url ahora /rxnpwa (launcher), name más genérico "RXN PWA — Apps Mobile".',
                'public/css/rxnpwa.css: override de text-muted/p/ol/btn-outline-light dentro de .rxnpwa-geo-gate-content (banner GPS dev) para legibilidad sobre fondo dark.',
                'app/modules/RxnPwa/views/horas_shell.php + horas_form.php + presupuestos_shell.php + presupuesto_form.php: header con botón nuevo "Menú PWA" (📱 grid icon) que va a /rxnpwa, al lado de fullscreen y volver al backoffice. Permite saltar entre apps PWA sin tener que volver al backoffice.',
                'app/config/version.php: bump a 1.43.1 / build 20260502.2.',
            ],
        ],
        [
            'version' => '1.43.0',
        ...
        [
            ...
            'title' => 'Iteración 44 — Hotfix fullscreen FAB + toast centrado en form PWA',
            'summary' => 'Dos ajustes detectados al usar 1.42.1 en celu real. (1) El botón fullscreen embebido en la topbar del backoffice se ocultaba junto con la topbar al activar el modo faux — el operador quedaba sin forma de salir o upgradear a API real. Fix: el helper rxn-fullscreen.js inyecta automático un FAB (floating action button) fixed top-right que SÍ se ve cuando el faux está activo. Vive en <body> fuera de los headers ocultables. CSS escondido por default, visible solo cuando body tiene clase rxn-faux-fullscreen. Click hace toggle igual que cualquier otro botón con data-rxn-fullscreen-toggle. (2) En el form PWA el aviso "Borrador guardado localmente" vivía en un div arriba del form — si el operador estaba scrolleado abajo cargando renglones no veía la confirmación. Fix: showStatus() ahora pinta un toast centrado fixed top con animación slide-in, ancho 92vw / 460px máximo, autodescarte 2.5s para success/info, persistente con botón cerrar para warning/error. Funciona sin importar el scroll. SW bumpeado a v13.',
            'items' => [
                'public/js/rxn-fullscreen.js: ensureFab() inyecta button#rxn-fullscreen-fab.rxn-fullscreen-fab al body con data-rxn-fullscreen-toggle. Init lo llama después de applyPersistedPref.',
                'public/css/rxn-fullscreen.css: estilo .rxn-fullscreen-fab (fixed top-right, círculo dark con glass). display:none por default. body.rxn-faux-fullscreen .rxn-fullscreen-fab { display:inline-flex }.',
                'public/js/pwa/rxnpwa-form.js: showStatus reescrito a toast centrado en body, con ícono por tipo, persistencia diferenciada (success/info auto 2.5s, warning/error con botón cerrar). clearStatus oculta con animación.',
                'public/css/rxnpwa.css: estilos .rxnpwa-form-toast (fixed top, transform slide-in) y .rxnpwa-form-toast-card. Responsive 92vw / 460px máx.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v13-2026-04-30 para invalidar cache.',
                'app/config/version.php: bump a 1.42.2 / build 20260430.18.',
            ],
        ],
        [
            'version' => '1.42.1',
            'build' => '20260430.17',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 44 — Hotfix fullscreen: el segundo click upgradea de faux a API real',
            'summary' => 'Bug detectado al usar 1.42.0: la URL bar del browser seguía visible incluso después de tocar el botón fullscreen. Causa: cuando llegabas a una página con pref="on", el helper aplicaba el modo faux silencioso (sin user gesture la Fullscreen API real no se puede invocar). Después, al hacer click en el botón, el helper veía isActive()===true (porque faux estaba puesto) y disparaba el camino "salir", apagando la pref. Resultado: nunca se llegaba a invocar la API real que es la única que oculta la URL bar del browser. Fix en rxn-fullscreen.js: el toggle ahora distingue 3 estados — (a) API real activa → salir totalmente, (b) faux activo Y API soportada → UPGRADEAR a API real conservando pref="on", (c) ninguno activo → entrar normal. Resultado para el operador: primer click cuando entrás a una página oculta el header interno. Segundo click upgradea a fullscreen real del browser ocultando también la URL bar. Si seguís navegando y volvés, repetís: primer click esconde header, segundo upgradea a real. SW bumpeado a v12.',
            'items' => [
                'public/js/rxn-fullscreen.js: función toggle() reescrita con 3 caminos explícitos. Path (b) es el upgrade faux→API real con fallback (si la API real falla, salir del faux como segundo click conceptual). El estado API real se chequea con isApiFullscreen() en lugar del isActive() compuesto.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v12-2026-04-30 para invalidar cache del helper.',
                'app/config/version.php: bump a 1.42.1 / build 20260430.17.',
            ],
        ],
        [
            'version' => '1.42.0',
            'build' => '20260430.16',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 44 — Pantalla completa global y persistente (backoffice mobile + PWA)',
            'summary' => 'Pedido del rey: el toggle de pantalla completa tiene que estar en TODAS las superficies (no solo en el form de presupuestos PWA) y la elección tiene que persistir al ir entre módulos. Si lo activé en el dashboard CRM, al entrar a Presupuestos sigue activo; lo mismo en la PWA entre el shell y el form. Implementación: helper global público/js/rxn-fullscreen.js que centraliza Fullscreen API + fallback "faux fullscreen" (CSS body.rxn-faux-fullscreen que oculta los headers sticky) + persistencia en localStorage bajo clave rxn_fullscreen_pref. Cualquier botón con atributo data-rxn-fullscreen-toggle se conecta automático al helper — el ícono se refresca solo en todos los toggles bound cuando cambia el estado. Ubicaciones: (1) admin_layout.php carga el helper junto con el resto de scripts globales, así está disponible en TODO el backoffice. (2) backoffice_user_banner.php agrega botón fullscreen entre el hamburger y la campanita, visible solo en mobile (d-lg-none). (3) Shell PWA suma botón fullscreen en el header al lado del "Volver al backoffice". (4) Form PWA migra del JS local al helper compartido — el botón ahora usa data-rxn-fullscreen-toggle igual que los demás. CSS rxn-fullscreen.css concentra las reglas de oculten-header-sticky para .rxn-backoffice-topbar, #rxn-page-header y .rxnpwa-header. Cuando entrás a otra página el bootstrap silencioso aplica el modo faux (sin user gesture no se puede invocar la API real); cuando tocás el botón se intenta la API real y si falla cae al faux. SW bumpeado a v11.',
            'items' => [
                'public/js/rxn-fullscreen.js: helper nuevo. RxnFullscreen.{toggle,isActive,getPref,setPref}. Click delegado a [data-rxn-fullscreen-toggle]. Bootstrap aplica pref persistida en faux mode. Listeners fullscreenchange refrescan todos los botones bound.',
                'public/css/rxn-fullscreen.css: reglas faux-fullscreen para .rxn-backoffice-topbar, .rxn-user-banner, .rxn-mobile-topbar, #rxn-page-header, .rxnpwa-header. Padding adjust para el shell PWA cuando el header oculto.',
                'app/shared/views/admin_layout.php: <link> a rxn-fullscreen.css en el head + <script> a rxn-fullscreen.js antes de rxn-shortcuts (para que esté disponible early).',
                'app/shared/views/components/backoffice_user_banner.php: clase rxn-backoffice-topbar al wrapper para que el CSS la pueda ocultar. Botón btn-link con bi-fullscreen + data-rxn-fullscreen-toggle visible solo en d-lg-none, dentro del rxn-user-menu antes del notifications_bell.',
                'app/modules/RxnPwa/views/presupuestos_shell.php: <link> a rxn-fullscreen.css + <script> rxn-fullscreen.js cargado antes del geo-gate. Botón data-rxn-fullscreen-toggle en el header al lado del "Volver al backoffice".',
                'app/modules/RxnPwa/views/presupuesto_form.php: idem CSS y script. Botón en header migrado a data-rxn-fullscreen-toggle (perdió id="rxnpwa-fullscreen").',
                'public/js/pwa/rxnpwa-form.js: eliminada la sección "Fullscreen" local (toggleFullscreen/updateFullscreenIcon/isFullscreen) — la maneja el helper global.',
                'public/css/rxnpwa.css: borradas las reglas de body.rxnpwa-faux-fullscreen (migradas al helper global con clase rxn-faux-fullscreen).',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v11-2026-04-30 para invalidar cache.',
                'app/config/version.php: bump a 1.42.0 / build 20260430.16.',
            ],
        ],
        [
            'version' => '1.41.0',
            'build' => '20260430.15',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 44 — Web mobile-friendly: scroll del dashboard + bitácora minimizada',
            'summary' => 'Dos ajustes para que la web sea más respetuosa cuando se accede desde celu (no PWA, web normal). (1) El sortable de los dashboards (CRM, tenant Tiendas y Launcher home) interceptaba el touch al primer pixel — un swipe vertical para scrollear se interpretaba como inicio de drag y movía las cards. Ahora los 3 dashboards usan delay=250ms con delayOnTouchOnly=true y touchStartThreshold=5px: el scroll vertical normal nunca activa el drag, solo se activa con un long-press explícito. En desktop el comportamiento NO cambia (delayOnTouchOnly aplica solo a touch). (2) La bitácora interna (módulo notes panel, "Solo administradores") arrancaba expandida y tapaba parte del contenido en pantallas chicas. Ahora cuando el UA es mobile, server-side fuerza el estado is-collapsed por default — el operador ve el launcher chico abajo a la derecha y lo abre solo si lo necesita. En desktop conserva la lógica original (abre si hay flash o si el módulo todavía no tiene notas).',
            'items' => [
                'app/modules/Dashboard/views/crm_dashboard.php: Sortable suma { delay:250, delayOnTouchOnly:true, touchStartThreshold:5 }. Comentario explicando el motivo (scroll mobile).',
                'app/modules/Dashboard/views/home.php: idem en el Sortable del launcher principal.',
                'app/modules/Dashboard/views/tenant_dashboard.php: idem en el dashboard del entorno Tiendas.',
                'app/shared/views/components/module_notes_panel.php: detección server-side de UA mobile (regex Android|iPhone|iPad|Mobile|...). Si mobile, $moduleNotesShouldOpen=false sin importar la lógica original — la bitácora arranca minimizada. En desktop mantiene la lógica previa (abre si hay flash o el módulo está vacío).',
                'app/config/version.php: bump a 1.41.0 / build 20260430.15.',
            ],
        ],
        [
            'version' => '1.40.0',
            'build' => '20260430.14',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 44 — PWA: gate depósito + edición de renglones + pantalla completa',
            'summary' => 'Tres ajustes UX detectados al usar la PWA en celu real con Charly cargando pruebas. (1) Gate depósito obligatorio: hasta ahora se podía abrir el modal de agregar renglón sin haber elegido depósito en cabecera, lo que generaba renglones sin stock visible y precios sin contexto. Ahora si no hay depósito, el botón "Agregar" muestra mensaje de error, hace foco en el select de depósito (con clase is-invalid + scroll suave) y NO abre el modal. La label del depósito en cabecera lleva asterisco rojo y atributo required, alineado con Lista y Clasificación. (2) Edición de renglones: para presupuestos largos era frustrante tener que borrar + reagregar para cambiar una cantidad o un precio. Ahora cada renglón muestra al lado del trash un botón lápiz que reabre el mismo modal con título "Editar renglón", pre-cargado con el artículo, cantidad, precio y descuento del renglón existente. Confirmar guarda in-place preservando row_uuid (idempotencia del sync). Si el artículo del renglón ya no existe en catálogo offline (caso raro: catálogo se resincronizó y el ítem se removió), se reconstruye un objeto mínimo desde el draft para que la edición no rompa. (3) Pantalla completa: nuevo botón en el header del form (a la izquierda del Guardar) con ícono fullscreen que llama a Fullscreen API estándar del browser. En iOS Safari (que no soporta FS para documentos arbitrarios) cae a un modo "faux fullscreen" via clase CSS en body que oculta el header sticky de la PWA — gana espacio vertical equivalente. Listener fullscreenchange refresca el ícono cuando el operador sale con ESC. SW bumpeado a v10 para invalidar cache.',
            'items' => [
                'app/modules/RxnPwa/views/presupuesto_form.php: header del form lleva botón btn-outline-light con bi-fullscreen a la izquierda del Guardar. Label "Depósito" suma asterisco rojo y atributo required en el select. Modal de renglón cambia title id="rxnpwa-renglon-modal-title" + label dinámico id="rxnpwa-renglon-confirm-label" para soportar texto distinto entre Agregar y Editar.',
                'public/js/pwa/rxnpwa-form.js: openRenglonModal(idx) acepta argumento opcional. Si idx es número >=0, entra en modo EDIT con pre-carga del artículo + cantidad + precio + descuento + stock. Sin argumento, modo ADD. Gate de depósito al inicio: si no hay deposito_codigo, showStatus error, foco/scroll/is-invalid en el select y return. Variable editingRenglonIdx (-1 = add). confirmRenglon detecta el modo y hace push o update in-place preservando row_uuid. onRenglonListClick suma rama [data-rxnpwa-renglon-edit] que llama openRenglonModal(idx). renderRenglones agrega botón btn-outline-primary con lápiz al lado del trash. Helpers nuevos toggleFullscreen/updateFullscreenIcon/isFullscreen con fallback faux-fullscreen para iOS Safari.',
                'public/css/rxnpwa.css: regla body.rxnpwa-faux-fullscreen .rxnpwa-header { display:none } + ajuste de padding del shell para recuperar vertical sin header sticky en iOS.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v10-2026-04-30 para invalidar cache de los nuevos JS/CSS.',
                'app/config/version.php: bump a 1.40.0 / build 20260430.14.',
            ],
        ],
        [
            'version' => '1.39.0',
            'build' => '20260430.13',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 44 — PWA: hardening + UX + bypass dev local sin SSL',
            'summary' => 'Cuatro frentes en la PWA mobile post-1.38.0. (1) Hardening Fase 3 que había quedado pendiente: los 3 endpoints POST (/api/rxnpwa/presupuestos/sync, /api/rxnpwa/presupuestos/{id}/attachments, /api/rxnpwa/presupuestos/{id}/emit-tango) ahora exigen header X-CSRF-Token validado contra la sesión y aplican rate limit por (scope, user, empresa) — sync 60/min, upload 120/min, emit-tango 20/min. El cliente lee el meta csrf-token del shell/form y lo manda en cada fetch. Si rate limit excede, response 429 con retry_after. Si CSRF falla, 403 con mensaje claro pidiendo recargar la PWA. (2) GC automático de drafts ya entregados al server: los drafts con status synced/emitted más viejos que 7 días se borran de IndexedDB al cargar el shell (el presupuesto server-side queda intacto). Nuevo botón en la sección "Cola de envío": "Limpiar N enviados del celu" que purga inmediato. (3) Card "PWA — Presupuestos Mobile" del dashboard CRM ahora se oculta server-side cuando el UA NO es mobile — sólo aparece desde celu (donde tiene sentido). En desktop el card duplicaba el banner azul superior; quedaba ruidoso. (4) Header del shell PWA rediseñado: 2 cajas en grid arriba (col-7 estado del catálogo + col-5 botón Sincronizar), eliminando la card sólo-botón que estaba abajo. Badge del catálogo más compacto (sin alert de Bootstrap), tres líneas: estado coloreado / items+tamaño / fecha de sync. (5) Fix crítico para pruebas en LAN sin SSL: cuando window.isSecureContext es false (HTTP plano, ej http://192.168.10.10:9021), navigator.geolocation está bloqueado por el browser y devuelve "denied" sin siquiera mostrar prompt — el gate quedaba trancado. Ahora detectamos el contexto inseguro y mostramos un overlay específico con botón "Activar GPS simulado (sólo dev)" que asigna lat/lng fijo (Obelisco BA) con source="dev_mock". Banner amarillo persistente arriba durante toda la sesión recordando que la geo es ficticia. En PRODUCCIÓN (HTTPS) el bypass NUNCA se ofrece — el gate sigue exigiendo GPS real. (6) NOTA INTERNA: los archivos MODULE_CONTEXT.md están viajando en el ZIP del OTA por error — son docs internas y NO deben subir a prod. Ajuste pendiente para la próxima OTA: excluirlos del ReleaseBuilder.',
            'items' => [
                'app/modules/RxnPwa/RxnPwaController.php: imports CsrfHelper + RateLimiter. Métodos privados checkCsrf() (valida HTTP_X_CSRF_TOKEN, 403 si falla) y checkRateLimit($scope, $max, $window) que combina (scope, user_id, empresa_id) y responde 429 con Retry-After + retry_after en el body. Aplicado a syncPresupuesto (60/min), uploadAttachment (120/min) y emitTango (20/min).',
                'public/js/pwa/rxnpwa-sync-queue.js: helper getCsrfToken() lee meta[name=csrf-token]. postJson y uploadAttachment mandan X-CSRF-Token en cada request. Manejo explícito de 401/403/429 con mensajes accionables.',
                'public/js/pwa/rxnpwa-drafts-store.js: nuevas funciones garbageCollectSynced(daysOld=7) y purgeAllSynced() que borran drafts con status synced/emitted (más sus attachments) sin tocar el server.',
                'public/js/pwa/rxnpwa-shell-drafts.js: GC automático al DOMContentLoaded (best-effort, falla silente). Botón "Limpiar N enviados del celu" en la sección Cola de envío cuando hay drafts purgables.',
                'app/modules/Dashboard/views/crm_dashboard.php: detección server-side de UA mobile (regex Android|iPhone|iPad|Mobile|...) — si NO es mobile, hace unset($defaultCards["pwa_presupuestos"]). El banner azul de invitación a la PWA sigue apareciendo en mobile como antes.',
                'app/modules/RxnPwa/views/presupuestos_shell.php: header del shell con div.row.g-2 — col-7 contiene el div#rxnpwa-status (badge del catálogo) y col-5 contiene el botón Sincronizar (full-width dentro de su card). Eliminado el #rxnpwa-actions independiente.',
                'public/js/pwa/rxnpwa-register.js: renderBadge() reescrito para devolver markup compacto sin alert de Bootstrap — fwbold + colored title / items+size / fecha. Encaja en la card chica del col-7.',
                'public/css/rxnpwa.css: .rxnpwa-header-grid (margin-bottom 0 en cards anidadas), .rxnpwa-badge-meta como clase global con tipografía 0.72rem. .rxnpwa-dev-mock-banner (sticky top, fondo ámbar) para el modo dev.',
                'public/js/pwa/rxnpwa-geo-gate.js: helper isInsecureContext(). captureAndGate detecta contexto inseguro y muestra overlay con título "Servidor sin HTTPS" + botón warning "Activar GPS simulado". Mock = {lat:-34.6037, lng:-58.3816, accuracy:50, source:"dev_mock"}. Banner amarillo sticky mientras el modo dev esté activo. setInterval skip-refresh si la fuente es dev_mock.',
                'public/js/pwa/rxnpwa-sync-queue.js: VALID_SOURCES suma "dev_mock" SOLO si isInsecureContext() — en prod (HTTPS) sigue requiriendo gps/wifi.',
                'public/js/pwa/rxnpwa-form.js: captureGeoIfMissing() acepta source dev_mock con la misma protección dev-only.',
                'app/modules/RxnGeoTracking/GeoTrackingService.php: $allowedSources suma "dev_mock" para que el server persista sin coercionar a "error" cuando viene de pruebas locales sin SSL.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v9-2026-04-30 para invalidar cache de los nuevos JS/CSS.',
                'app/config/version.php: bump a 1.39.0 / build 20260430.13.',
                '⚠️ PENDIENTE PRÓXIMA OTA: excluir app/modules/*/MODULE_CONTEXT.md del ZIP del release. Son docs internas que no deben viajar a prod (ya guardado en memoria persistente).',
            ],
        ],
        [
            'version' => '1.38.0',
            'build' => '20260430.12',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — PWA: GPS obligatorio para usar la app (gate global bloqueante)',
            'summary' => 'Pedido del rey: la PWA es para tracking de presupuestos en campo, sin GPS no se puede operar. Diferencial fuerte vs apps de la competencia. Implementación: módulo nuevo rxnpwa-geo-gate.js que se carga PRIMERO en shell y form mobile. Al entrar a la PWA pide acceso a la ubicación; si falla (denegado, timeout, error, no soportado) muestra un overlay bloqueante a pantalla completa con mensaje claro, instrucciones para activar el GPS y botón "Reintentar". El usuario no puede tocar nada de la PWA hasta que dé permiso. Refresh automático cada 5 minutos para detectar si el GPS se desactivó durante el uso. Los módulos rxnpwa-form.js y rxnpwa-sync-queue.js se refactorizaron para reusar el gate central — ya no piden geo por su cuenta, leen window.RxnPwaGeoGate.getCurrentGeo(). Cuando se emite a Tango, la queue valida que la geo del draft venga de fuente confiable (gps o wifi); si no, fuerza retry del gate y, si sigue inválida, tira excepción explicando por qué. La auditoría server-side via RxnGeoTracking queda con lat/lng del celu real, accuracy en metros, y source para distinguir entre captura precisa vs degraded. Confirm dialog de Tango ahora muestra las coordenadas y la precisión antes de mandar.',
            'items' => [
                'public/js/pwa/rxnpwa-geo-gate.js: módulo nuevo. RxnPwaGeoGate.getCurrentGeo() y retry() expuestos globalmente. captureAndGate al DOMContentLoaded + setInterval cada 5 min. Overlay bloqueante (rxnpwa-geo-gate) con instrucciones detalladas para activar GPS según browser/SO.',
                'public/css/rxnpwa.css: estilos del gate (.rxnpwa-geo-gate fixed inset-0 z-9999 con backdrop blur).',
                'app/modules/RxnPwa/views/presupuestos_shell.php + presupuesto_form.php: cargan rxnpwa-geo-gate.js como PRIMER script PWA, antes de catalog/drafts/sync/form.',
                'public/js/pwa/rxnpwa-form.js: captureGeoIfMissing simplificado — copia la geo actual del gate al draft. Ya no pide permisos por su cuenta.',
                'public/js/pwa/rxnpwa-sync-queue.js: emitToTango valida geo válida (gps/wifi) antes de mandar; si no, fuerza retry del gate, si sigue inválida tira excepción con razón clara. captureGeoForDraft refactorizado a usar el gate.',
                'public/js/pwa/rxnpwa-form-sync.js: simplificado el confirm de Tango (la queue se encarga del gate ahora). El usuario ve mensaje claro si GPS no está disponible.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v8-2026-04-30.',
                'app/config/version.php: bump a 1.38.0 / build 20260430.12.',
            ],
        ],
        [
            'version' => '1.37.1',
            'build' => '20260430.11',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — Hotfix: warning filemtime + form mobile en blanco',
            'summary' => 'Bug en _brand_icon.php (release 1.37.0): el orden de evaluación de filemtime() era erróneo — se llamaba filemtime() del archivo ANTES de chequear is_file(). Como el archivo rxnpwa-source.png no existe todavía, PHP emitía warning "stat failed" y rompía la salida HTML, dejando el form mobile completamente en blanco. Fix: reordenar para chequear is_file PRIMERO y solo entonces componer la URL versionada con filemtime. SW bumpeado a v7 para invalidar cache.',
            'items' => [
                'app/modules/RxnPwa/views/_brand_icon.php: chequea is_file($sourceAbs) primero; sólo si existe llama filemtime para la cache-bust query string. Sino, $hasSource=false y se renderiza el SVG inline.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v7-2026-04-30.',
                'app/config/version.php: bump a 1.37.1 / build 20260430.11.',
            ],
        ],
        [
            'version' => '1.37.0',
            'build' => '20260430.10',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — PWA: ícono RXN + título Presupuestos + geolocalización del presupuesto offline + fix modal artículo',
            'summary' => '4 ajustes finales sobre la PWA. (1) Ícono RXN del header — SVG inline aproximando la estrella RXN (8 puntas alternadas blanco/rojo + flash central); robusto, no depende de archivos. Cuando Charly drope public/icons/rxnpwa-source.png con el arte final, el partial _brand_icon.php detecta el archivo y lo usa automáticamente en lugar del SVG inline. Aplica al shell y al form mobile. (2) Subtítulo del header pasa de "Presupuestos en campo" a "Presupuestos" — anticipa que vendrán otros módulos PWA (Pedidos de Servicio, etc) y el subtítulo identifica el módulo activo. (3) Bug modal de artículo: al abrir aparecía el listado completo automáticamente saturando el modal. Ahora el listado solo aparece al hacer focus/click en el input (mismo patrón que cliente picker en el form). (4) Geolocalización del presupuesto creado offline en la PWA — el celu captura GPS al primer guardar manual y antes del sync, persiste lat/lng/accuracy/source en el draft, y los manda al server. El RxnPwaController, después de crear el presupuesto en DB, llama a RxnGeoTracking::registrar(EVENT_PRESUPUESTO_CREATED) y luego reporta la posición real del celu via reportarPosicionBrowser — la misma data que captura el form web pero ahora capturando el lugar real en campo donde el vendedor cotizó. Si el usuario deniega permiso, queda registrado source="denied" para auditoría.',
            'items' => [
                'app/modules/RxnPwa/views/_brand_icon.php: partial nuevo. Si existe public/icons/rxnpwa-source.png lo usa como <img>; sino renderiza SVG inline con 8 triángulos alternados blanco/rojo + flash central (aproximación de la estrella RXN).',
                'app/modules/RxnPwa/views/presupuestos_shell.php + presupuesto_form.php: requieren _brand_icon.php en el header. Subtítulo "Presupuestos" en lugar de "Presupuestos en campo".',
                'public/js/pwa/rxnpwa-form.js: openRenglonModal limpia el listado en lugar de pre-renderizarlo (aparece al focus/click). Función nueva captureGeoIfMissing() con timeout 8s y enableHighAccuracy. saveDraft() la dispara al primer guardar manual (no auto-saves silenciosos para no insistir con el prompt). API pública window.RxnPwaForm.captureGeo expuesta.',
                'public/js/pwa/rxnpwa-form-sync.js: antes de enqueue, llama a window.RxnPwaForm.captureGeo() para garantizar que la geo viaja al server.',
                'public/js/pwa/rxnpwa-sync-queue.js: draftToWire suma campo "geo" {lat, lng, accuracy, source, captured_at}.',
                'app/modules/RxnPwa/RxnPwaController.php: syncPresupuesto llama recordGeoEvent() después de crear. Método nuevo registra el evento en RxnGeoTracking y reporta la posición precisa del celu via reportarPosicionBrowser. Fail-silent — geo no rompe el sync.',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v6-2026-04-30 para invalidar cache.',
                'app/config/version.php: bump a 1.37.0 / build 20260430.10.',
            ],
        ],
        [
            'version' => '1.36.0',
            'build' => '20260430.9',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — PWA: ajustes UX (header compacto + pickers con vistazo + stock visible en artículo)',
            'summary' => '4 ajustes UX detectados al usar la PWA en celu real. (1) Ícono del header — el placeholder PNG no aparecía; reemplazado por gradiente circular con bi-phone-fill (siempre visible, no depende del SW). (2) Cards "Catálogo al día" y "Preparar app" ocupaban demasiado vertical; ahora son una sola fila compacta con badge resumen + botón Sincronizar chico al lado. (3) Cliente picker — antes había que escribir para que aparezca algo; ahora al hacer focus o tap muestra los primeros 30 clientes con header "Primeros N de M — escribí para buscar", mismo patrón que el spotlight web. (4) Artículo picker — idem + badge de stock visible en cada resultado del catálogo (verde si stock > 0, gris si 0/sin info), respeta el depósito seleccionado en cabecera. Pre-render al abrir el modal sin esperar focus. SW bumpeado a v5 para invalidar cache de assets PWA.',
            'items' => [
                'app/modules/RxnPwa/views/presupuestos_shell.php: header usa span.rxnpwa-brand-icon (gradiente + bi-phone-fill) en lugar del <img>. Card "Preparar app" colapsada a una sola fila con botón btn-sm. Status alert reducido a py-2 + small.',
                'public/js/pwa/rxnpwa-register.js: renderBadge() en una sola línea (badge título + meta inline). Padding y margin reducidos para ahorrar vertical.',
                'public/js/pwa/rxnpwa-form.js: cliente picker suma listeners focus + click → renderClienteResults(""). Sin query muestra primeros 30 con header "Primeros N de M". Idem artículo picker. Artículo picker suma badge de stock por resultado usando resolveStock(art, deposito_codigo). openRenglonModal pre-renderiza primeros 30 al abrir.',
                'public/css/rxnpwa.css: rxnpwa-card-compact (padding 0.5/0.75rem), rxnpwa-brand-icon (gradiente azul/violeta circular), rxnpwa-status-compact (.badge-meta inline), rxnpwa-suggest-header (sticky en suggestions).',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v5-2026-04-30 para invalidar cache de los nuevos JS/CSS.',
                'app/config/version.php: bump a 1.36.0 / build 20260430.9.',
            ],
        ],
        [
            'version' => '1.35.2',
            'build' => '20260430.8',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — Hotfix crítico PWA: ID_GVA01/10/23/24 mal mapeados al enviar a Tango',
            'summary' => 'Tango rechazaba todos los presupuestos sincronizados desde la PWA con "No existe condición de venta para el ID_GVA01 ingresado: <N>". Causa: en RxnPwaSyncService::resolveCatalogItem yo estaba poniendo $row["id"] (el PK auto-increment de crm_catalogo_comercial_items) como id_interno, pero el id_interno real es la columna `id_interno` de esa tabla (que mapea contra ID_GVA01 de Tango). Esto afectaba los 4 campos comerciales (condición, lista, vendedor, transporte) — todos viajaban con un ID que no existe en Tango. Fix de una línea en el service: leer $row["id_interno"] como hace el form web. Los presupuestos YA enviados desde la PWA con este bug están persistidos con condicion/lista/vendedor/transporte_id_interno incorrectos en DB; para mandarlos a Tango hay que abrir cada uno desde el form web, refrescar la cabecera (cliente_id_interno se re-resuelve) y volver a enviar.',
            'items' => [
                'app/modules/RxnPwa/RxnPwaSyncService.php: resolveCatalogItem ahora lee $row["id_interno"] (columna que mapea contra ID_GVA01/10/23/24 de Tango) en lugar de $row["id"] (PK auto local). Comentario explicando el motivo para que no se repita.',
                'app/config/version.php: bump a 1.35.2 / build 20260430.8.',
            ],
        ],
        [
            'version' => '1.35.1',
            'build' => '20260430.7',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — Hotfix: copia de presupuesto rompía por tmp_uuid_pwa duplicado + invalidación automática del catálogo PWA',
            'summary' => '2 fixes encontrados durante el smoke test. (1) Copiar un presupuesto desde el form web fallaba con "Duplicate entry TMP-... for key uniq_crm_presupuestos_tmp_uuid_pwa". El controller `copy()` arrastraba el tmp_uuid_pwa del original al INSERT del nuevo; el UNIQUE explotaba porque ese UUID ya existía. Fix: `unset($data["tmp_uuid_pwa"])` antes del create — la copia es un presupuesto independiente desde la web, no tiene draft mobile asociado. `createNewVersion` ya construía el data manualmente sin incluir el campo, no necesitó cambio. (2) Defaults comerciales del cliente no aparecían en el celu después del bump 1.35.0 porque IndexedDB seguía con el shape viejo (sin las nuevas columnas id_gva*). Fix sistémico: sumamos CATALOG_SCHEMA_VERSION="v2" al store y al guardar metadata. Tanto register.js (shell) como form.js (form mobile) detectan ahora schema_version mismatch y wipean automáticamente el catálogo + muestran badge "Hay una versión nueva del catálogo (con más datos del cliente). Resincronizá". Drafts y attachments NO se tocan — la limpieza es solo del catálogo. Bumpeamos el SW a v4 para invalidar el cache de los JS también.',
            'items' => [
                'app/modules/CrmPresupuestos/PresupuestoController.php: copy() agrega `tmp_uuid_pwa` al unset junto con id/numero. Comentario explicando el motivo.',
                'public/js/pwa/rxnpwa-catalog-store.js: nueva constante CATALOG_SCHEMA_VERSION="v2". saveCatalog persiste schema_version en meta. Método nuevo clearCatalogOnly() que limpia stores del catálogo + meta sin tocar drafts/attachments.',
                'public/js/pwa/rxnpwa-register.js: ensureEmpresaConsistency renombrada a ensureCatalogConsistency, devuelve "empresa"/"schema"/null. Renderiza badge específico para cada caso. Usa clearCatalogOnly().',
                'public/js/pwa/rxnpwa-form.js: loadCatalog detecta schema mismatch además del empresa mismatch, redirige al shell con mensaje claro. Usa clearCatalogOnly().',
                'public/sw.js: RXNPWA_VERSION bumpeado a rxnpwa-v4-2026-04-30 para invalidar cache de assets PWA.',
                'app/config/version.php: bump a 1.35.1 / build 20260430.7.',
            ],
        ],
        [
            'version' => '1.35.0',
            'build' => '20260430.6',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — PWA: defaults comerciales del cliente (condición + vendedor + transporte) auto-completados',
            'summary' => 'Sin estos defaults el envío a Tango fallaba porque el ERP exige condición de venta + vendedor + lista. Ahora la PWA replica el comportamiento del form web: al seleccionar cliente, autocompleta los 4 campos comerciales (lista de precios, condición de venta, vendedor, transporte) leyendo los códigos configurados en la fila del cliente offline. (1) Catálogo offline ampliado: fetchClientes ahora trae además de razon_social/documento/email los IDs comerciales — id_gva01_condicion_venta, id_gva10_lista_precios, id_gva23_vendedor, id_gva24_transporte y los fallback id_gvaXX_tango. (2) Form mobile suma 3 selects nuevos (Condición de venta, Vendedor, Transporte) en cabecera, junto a los ya existentes Lista y Depósito. Helper renderCatalogSelect compartido detecta empty (catálogo no sincronizado → disabled con texto explicativo) y maneja códigos huérfanos (cliente con código viejo que ya no existe en catálogo → se conserva la selección como "no encontrado en catálogo"). (3) applyClienteDefaults() replica el clientContext del backoffice: al seleccionar cliente, recorre los 4 campos con cadena de fallback (campo CRM nativo → campo legacy Tango → vacío) y escribe sólo los que están vacíos en el draft (no pisa elecciones manuales del operador). Si la lista cambió como parte del autofill, recalcula precios de los renglones existentes. Mensaje de feedback debajo de la cabecera mostrando qué defaults se aplicaron, o avisando "este cliente no tiene defaults comerciales configurados" si vino vacío. (4) RxnPwaSyncService persiste los 4 campos al server: resuelve cada uno contra crm_catalogo_comercial_items (igual que lista/depósito) y los pasa al payload de PresupuestoRepository::create. Ahora el envío a Tango desde mobile arranca con los mismos campos resueltos que el form web. Smoke test pendiente del rey: cargar cliente con defaults configurados → verificar que aparecen los 4 selects con valores.',
            'items' => [
                'app/modules/RxnPwa/RxnPwaCatalogService.php: fetchClientes amplía SELECT con id_gva01_condicion_venta, id_gva10_lista_precios, id_gva23_vendedor, id_gva24_transporte + fallbacks id_gva23_tango/id_gva10_tango/id_gva01_tango/id_gva24_tango.',
                'app/modules/RxnPwa/views/presupuesto_form.php: 3 selects nuevos en cabecera (rxnpwa-condicion, rxnpwa-vendedor, rxnpwa-transporte). Mensaje de feedback rxnpwa-cliente-defaults-msg debajo del bloque de cabecera.',
                'public/js/pwa/rxnpwa-form.js: renderCondicionSelect/renderVendedorSelect/renderTransporteSelect via helper renderCatalogSelect compartido. Listeners de change que actualizan draft.cabecera.{condicion,vendedor,transporte}_codigo + autoSave. applyClienteDefaults() con cadena de fallback por campo, sólo escribe campos vacíos del draft. Refresca selects + recalcula precios de renglones si la lista cambió.',
                'app/modules/RxnPwa/RxnPwaSyncService.php: buildPayload resuelve condicion_venta/vendedor/transporte con resolveCatalogItem y los suma al payload del repo. Antes iban hardcoded null.',
                'app/config/version.php: bump a 1.35.0 / build 20260430.6.',
            ],
        ],
        [
            'version' => '1.34.0',
            'build' => '20260430.5',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — PWA: detección de cambio de empresa + acceso desde dashboard CRM + banner mobile',
            'summary' => 'Patch sobre la PWA recién entregada. Resuelve 2 fricciones detectadas en el primer smoke test mobile y suma el acceso natural desde el backoffice. (1) Detección de cambio de empresa — bug reportado: al cambiar de empresa en el backoffice, el form mobile seguía mostrando catálogos de la empresa anterior y no aparecían las clasificaciones (porque venían de otro tenant). Fix: el shell PWA ahora compara meta.empresa_id del catálogo offline contra <meta name="rxn-empresa-id"> del backend y, si difieren, wipea el catálogo offline + muestra "Cambiaste de empresa — descargá el nuevo". El form mobile aplica la misma verificación al cargar; si detecta mismatch, limpia y redirige al shell. (2) Mensaje claro cuando una empresa NO tiene clasificaciones sincronizadas server-side — el select queda disabled con texto "Sin clasificaciones — corré Sync Catálogos en el backoffice", evitando confusión. (3) Card nueva "PWA — Presupuestos Mobile" en el dashboard CRM (icono celular), link directo a /rxnpwa/presupuestos. (4) Banner inteligente en el dashboard CRM: solo aparece si el navigator.userAgent matchea Android/iOS/Mobile, invita a abrir la versión PWA con un botón destacado. Botón X para descartar guarda flag en sessionStorage para no insistir durante esa sesión. NO redirige automático — el usuario decide.',
            'items' => [
                'public/js/pwa/rxnpwa-register.js: ensureEmpresaConsistency() lee <meta name=rxn-empresa-id> y compara con meta.empresa_id del catálogo offline. Si difieren, llama RxnPwaCatalogStore.clear() y renderiza badge específico "Cambiaste de empresa". Disparado en cada refreshStatus() — incluido al volver online y al abrir el shell.',
                'public/js/pwa/rxnpwa-form.js: loadCatalog() suma chequeo equivalente. Si detecta mismatch, wipea + redirect a /rxnpwa/presupuestos. renderClasificacionSelect() ahora detecta empty array y muestra select disabled con mensaje "Sin clasificaciones — corré Sync Catálogos".',
                'app/modules/Dashboard/views/crm_dashboard.php: card nueva pwa_presupuestos en defaultCards (icono bi-phone, link /rxnpwa/presupuestos). Banner div oculto por default + script inline que detecta UA mobile (regex Android|iPhone|iPad|Mobile|...) y muestra el banner; sessionStorage rxn_dismiss_pwa_banner descarta para esa sesión. Sin redirect automático.',
                'app/config/version.php: bump a 1.34.0 / build 20260430.5.',
            ],
        ],
        [
            'version' => '1.33.0',
            'build' => '20260430.4',
            'released_at' => '2026-04-30',
            'title' => 'Iteración 43 — RXN PWA Fase 3: sync queue + reconciliación + envío a Tango desde mobile',
            'summary' => 'Cierra el roadmap PWA (Bloque C). Los presupuestos creados offline en el celu ahora pueden viajar al server cuando hay red, con reintentos automáticos y un botón explícito para emitirlos a Tango sin pasar por la web. Lo que se entrega: (1) Endpoint POST /api/rxnpwa/presupuestos/sync que recibe el draft completo (cabecera + renglones + tmp_uuid) y lo persiste reusando PresupuestoRepository::create. Idempotente por columna nueva crm_presupuestos.tmp_uuid_pwa (VARCHAR 50 UNIQUE) — si llega 2 veces el mismo UUID, devuelve el id_server existente sin duplicar. Service nuevo RxnPwaSyncService encapsula el mapping del JSON del cliente al payload del repo, resolviendo catálogos por código y calculando totales server-side (no se confía en el cliente). (2) Endpoint POST /api/rxnpwa/presupuestos/{id}/attachments que sube cada adjunto del draft uno por uno reusando el AttachmentService polimórfico (owner_type=crm_presupuesto, ya whitelisteado en config). Multipart, valida MIME y blacklist por extensión via UploadValidator. IDOR: chequea que el presupuesto pertenezca a la empresa antes de aceptar el archivo. (3) Endpoint POST /api/rxnpwa/presupuestos/{id}/emit-tango que internamente llama PresupuestoTangoService::send — exactamente el mismo path que el form web, sin lógica nueva del lado server. (4) Cola de sync client-side (rxnpwa-sync-queue.js) con flujo 2-step: primero header+items, después attachments uno por uno. Reintentos automáticos hasta 5 con backoff exponencial 1s/2s/4s/8s/16s. Tras agotar reintentos, el draft queda en status="error" permanente y aparece botón "Reintentar" que vuelve a encolarlo desde 0. La cola es idempotente por kick() — múltiples llamadas concurrentes solo arrancan UNA pasada. (5) Estados visibles del draft: draft / pending_sync / syncing / synced / emitted / error. Reflejados como badges en el shell + en el form mobile, con acciones contextuales por card (Editar, Sincronizar, Reintentar, Enviar a Tango, Eliminar). (6) Auto-arranque de la cola en 3 triggers: DOMContentLoaded del shell o del form, evento online en window al recuperar red, y Background Sync API via SW (event tag rxnpwa-sync-queue → SW postMessage al cliente → kick). El Background Sync no está en iOS Safari pero el online event lo cubre. (7) Botón "Enviar a Tango" en el form mobile y en cada card del shell, deshabilitado si offline o si el draft no está synced. La política es deliberada: el draft debe sincronizarse al server PRIMERO (sin id_server no hay Tango); el usuario decide manualmente cuándo emitir. (8) Wrapper rxnpwa-form-sync.js que se monta sobre rxnpwa-form.js (1800 líneas) sin tocarlo: lee tmp_uuid del data-attribute del main, se suscribe a eventos de la queue y refresca el estado del form. Antes de encolar, llama a window.RxnPwaForm.flushSave() para garantizar que el último cambio está persistido. (9) Service Worker bumpeado a rxnpwa-v3 con handler nuevo del evento sync (Background Sync API) que postMessage-a a los clientes activos para drenar cola; si no hay clientes (PWA cerrada), la cola se procesa al abrir. (10) Script nuevo tools/generate_rxnpwa_icons_from_source.php que toma public/icons/rxnpwa-source.png (la estrella RXN que pasó el rey) y genera 192/512 con safe-area de 12% para máscaras circulares — sigue pendiente que Charly drope la fuente. Smoke test pendiente: end-to-end mobile real (crear draft offline → sincronizar al volver red → enviar a Tango).',
            'items' => [
                // === Migración ===
                'database/migrations/2026_04_30_02_alter_crm_presupuestos_add_tmp_uuid_pwa.php: ALTER que suma columna tmp_uuid_pwa VARCHAR(50) NULL + UNIQUE KEY uniq_crm_presupuestos_tmp_uuid_pwa. NULL no cuenta como duplicado en MySQL así que los presupuestos pre-PWA conviven sin chocar.',

                // === Backend ===
                'app/modules/RxnPwa/RxnPwaSyncService.php: service nuevo. syncDraft($draft, $empresaId, $usuarioId, $usuarioNombre) idempotente por findByTmpUuidPwa, mapea cabecera+renglones via resolveCatalogItem (reusa CommercialCatalogRepository::findOption) y calcula totales server-side. Maneja race condition: si el UNIQUE explota entre find y create, reintenta el find.',
                'app/modules/RxnPwa/RxnPwaController.php: 3 endpoints nuevos — syncPresupuesto (POST sync), uploadAttachment (POST attachments con AttachmentService polimórfico) y emitTango (POST emit-tango con PresupuestoTangoService::send). Validación de método, JSON body, IDOR por empresa.',
                'app/modules/CrmPresupuestos/PresupuestoRepository.php: sumado tmp_uuid_pwa al INSERT del create() + buildHeaderPayload + ALTER defensivo idempotente. Método nuevo findByTmpUuidPwa(string $tmpUuid, int $empresaId): ?array. Update unset el placeholder para no pisarlo post-creación (igual que version_padre_id).',

                // === Frontend ===
                'public/js/pwa/rxnpwa-sync-queue.js: cola con flujo 2-step (sync header → upload attachments) + backoff exponencial 5 reintentos + Background Sync registration + emitToTango. API: enqueue, kick, emitToTango, subscribe, registerBackgroundSync. Eventos pub/sub para que la UI re-renderice en tiempo real.',
                'public/js/pwa/rxnpwa-form-sync.js: wire-up de la sección "Enviar al servidor" del form mobile. Botones Sincronizar / Reintentar / Enviar a Tango con estados habilitados según status del draft + navigator.onLine. Llama window.RxnPwaForm.flushSave() antes de encolar. Suscrito a eventos de la queue para refresco reactivo.',
                'public/js/pwa/rxnpwa-form.js: expone API pública mínima window.RxnPwaForm.flushSave() y getDraft() — usadas por rxnpwa-form-sync.js. Sin tocar la lógica monolítica de 1800 líneas.',
                'public/js/pwa/rxnpwa-shell-drafts.js: rewrite. Listado de drafts con cards interactivas (botones Editar/Sincronizar/Reintentar/Tango/Eliminar contextuales por estado). Sección "Cola de envío" con resumen por bucket (pending/syncing/error/synced) + badge de red. Suscrito a la queue para refresco reactivo.',

                // === Vistas ===
                'app/modules/RxnPwa/views/presupuesto_form.php: reemplazado el placeholder Fase 3 con sección real "Enviar al servidor" — botón Sincronizar, botón Tango (gateado), badge de red, mensaje de estado. Carga rxnpwa-sync-queue.js + rxnpwa-form-sync.js.',
                'app/modules/RxnPwa/views/presupuestos_shell.php: reemplazado el placeholder de cola con la sección real con badge de red. Carga rxnpwa-sync-queue.js antes de rxnpwa-shell-drafts.js.',

                // === Service Worker + Routing ===
                'public/sw.js: RXNPWA_VERSION bumpeado a v3 (rxnpwa-v3-2026-04-30). Sumado handler self.addEventListener("sync", ...) para Background Sync con tag rxnpwa-sync-queue: postMessage rxnpwa-sync-queue-fire a todos los clientes activos. Web Push y resto del fetch handler intactos.',
                'app/config/routes.php: 3 rutas POST nuevas — /api/rxnpwa/presupuestos/sync, /api/rxnpwa/presupuestos/{id}/attachments, /api/rxnpwa/presupuestos/{id}/emit-tango.',

                // === Iconos ===
                'tools/generate_rxnpwa_icons_from_source.php: script nuevo que toma public/icons/rxnpwa-source.png (cuadrado, idealmente 1024×1024) y genera 192/512 con safe-area de 12% sobre fondo #0f172a. Pendiente que Charly drope el PNG fuente con la estrella RXN final.',

                // === Doc ===
                'app/modules/RxnPwa/MODULE_CONTEXT.md: actualizado a Fase 3. Sección Roadmap completa, Storage cliente con stores y campos nuevos, Sync queue 2-step, estados del draft, retry/backoff, auto-arranque, política de envío a Tango, seguridad transversal con notas sobre CSRF y rate limiting pendientes, Files completo, Pendiente actualizado.',

                // === Versión ===
                'app/config/version.php: bump a 1.33.0 / build 20260430.4.',
            ],
        ],
        [
            'version' => '1.32.0',
